    ...
    public void verifyConnectionLimitProfilesDisabledTableAction() throws MalformedURLException, FileNotFoundException, IOException {
        ...
        try {
            ...
            if (!(RBACDefenseProConnectionLimitProfilesTableActionHandler.verifyConnectionLimitProfilesTableAction(testProperties))) {
                report.report("The specified Table action is in an incorrect state: " + connectionLimitProfilesTableAction.getTableAction() + "\n.", Reporter.FAIL);
            }
        } catch (Exception e) {
            report.report("verify ConnectionLimitProfiles Disabled Table Action failed: " + connectionLimitProfilesTableAction.getTableAction() + "\n." + parseExceptionBody(e),
                    Reporter.FAIL);
        }
    }
    ...
    public void verifyConnectionLimitProtectionsDisabledTableAction() throws MalformedURLException, FileNotFoundException, IOException {
        ...
        try {
            ...
            if (!(RBACDefenseProConnectionLimitProfilesTableActionHandler.verifyConnectionLimitProtectionsTableAction(testProperties))) {
                report.report("The specified Table action is in an incorrect state: " + connectionLimitProtectionsTableAction.getTableAction() + "\n.", Reporter.FAIL);
            }
        } catch (Exception e) {
            report.report("verify ConnectionLimitProtections Disabled Table Action failed: " + connectionLimitProtectionsTableAction.getTableAction() + "\n." + parseExceptionBody(e),
                    Reporter.FAIL);
        }
    }
