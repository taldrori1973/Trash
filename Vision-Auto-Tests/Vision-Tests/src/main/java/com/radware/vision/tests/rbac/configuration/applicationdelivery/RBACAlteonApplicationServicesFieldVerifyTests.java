    ...
    public void verifySSLCertificateSubjectDefaultsFieldAction() throws MalformedURLException, FileNotFoundException, IOException {
        ...
        try {
            ...
            if (!(RBACAlteonApplicationServicesFieldActionHandler.verifySSLCertificateRepositoryFieldAction(testProperties))) {
                BaseTestUtils.report("The specified Field action is in an incorrect state: " + certificateSubjectDefaultsFieldAction.getTextValue() + "\n.", Reporter.FAIL);
            }
        } catch (Exception e) {
            BaseTestUtils.report("verify SSLCertificateSubjectDefaults Field Action failed: " + certificateSubjectDefaultsFieldAction.getTextValue() + "\n." + parseExceptionBody(e),
                    Reporter.FAIL);
        }
    }
