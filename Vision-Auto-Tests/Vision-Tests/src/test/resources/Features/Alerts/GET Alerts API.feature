...
Feature: GET Alert API
  ...
  Scenario:  Verify correct result for filter MODULE,SEVERITY,DEVICE TYPE
    ...
    # raisedTime
    # printf delimiter
    Then CLI Run linux Command "/home/radware/Scripts/GET_alert_API.sh "" "DEVICE_GENERAL" "INFO" "" "DEFENSE_PRO" "1" "2019-01-01" "message"" on "GENERIC_LINUX_SERVER" and validate result CONTAINS "locked by user sys_admin" with timeOut 30
  ...
  Scenario:  Verify correct result for filter NUMBER OF RESULTS
    ...
    Then Sleep "2"
    Then REST Lock Action on "DefensePro" 10
    Then CLI Run linux Command "/home/radware/Scripts/GET_alert_API.sh "" "DEVICE_GENERAL" "INFO" "" "DEFENSE_PRO" "1" "2019-01-01" "" |grep -o "locked by"|wc -l" on "GENERIC_LINUX_SERVER" and validate result EQUALS "2" with timeOut 30
