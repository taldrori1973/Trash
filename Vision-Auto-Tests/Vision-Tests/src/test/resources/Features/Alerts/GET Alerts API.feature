...
Feature: GET Alert API
  ...
  Scenario:  Clear data and set TACACS login
    ...
    # copy current vision management IP to the generix linux VM
  ...
  Scenario:  Verify correct result for filter MODULE,SEVERITY,DEVICE TYPE
    ...
    # raisedTime
    # printf delimiter
    Then CLI Run linux Command "/home/radware/Scripts/GET_alert_API.sh "" "DEVICE_GENERAL" "INFO" "" "DEFENSE_PRO" "1" "2019-01-01" "message"" on "GENERIC_LINUX_SERVER" and validate result CONTAINS "locked by user sys_admin"
  ...
  Scenario:  Verify correct result for filter NUMBER OF RESULTS
    ...
    Then REST Lock Action on "DefensePro" 10
    Then Sleep "12"
    Then CLI Run linux Command "/home/radware/Scripts/GET_alert_API.sh "" "DEVICE_GENERAL" "INFO" "" "DEFENSE_PRO" "1" "2019-01-01" "" |grep -o " locked by "|wc -l" on "GENERIC_LINUX_SERVER" and validate result EQUALS "2"
