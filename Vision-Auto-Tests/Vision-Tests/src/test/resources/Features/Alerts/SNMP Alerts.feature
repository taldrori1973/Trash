...
Feature: SNMP Alerts
  ...
  Scenario: IPv6 SNMP trap Cleanup
    ...
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alrt_fltr_to_categories where category=(select row_id from alertsfilter where name='Profile');""
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alrt_fltr_to_logical_grp_ids where alert_filter_id=(select row_id from alertsfilter where name='Profile');""
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alrt_fltr_to_ids where device_id=(select row_id from alertsfilter where name='Profile');""
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alrt_fltr_to_severities where severity=(select row_id from alertsfilter where name='Profile');""
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alrt_fltr_to_modules where module=(select row_id from alertsfilter where name='Profile');""
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alertsfilter where name='Profile';""
  ...
  @SID_2
  Scenario: SNMP trap login and create alert profile
    ...
    Then UI Set Text Field "Name" To "Profile"
    Then UI Click Button "Submit"

  @SID_3
  Scenario: SNMP trap Configure target server
    ...
    Then UI Set Text field with id "gwt-debug-name_Widget" with "Server"
    Then UI Set Text field with id "gwt-debug-ipAddress_Widget" with "172.17.178.20"
    Then UI Set Text field with id "gwt-debug-port_Widget" with "162"
    Then UI Set Text field with id "gwt-debug-community_Widget" with "public"
    Then UI Click Button "Submit"

  @SID_4
  Scenario: SNMP trap Configure rule
    Then UI Click Web element with id "gwt-debug-alertrule_NEW"
    Then UI Set Text Field "Name" To "Rule1"
    Then UI Select "Profile" from Vision dropdown by Id "gwt-debug-snmpProfile.name_Widget-input"
    Then UI Select "Server" from Vision dropdown by Id "gwt-debug-snmpTarget.name_Widget-input"
    Then UI Click Button "Submit"
    Then UI logout and close browser

  @SID_5
  Scenario: SNMP trap clear snmpd server log
    Then CLI Run remote linux Command "ssh root@172.17.178.20 'echo "cleared" $(date) > /var/log/snmptrap.log'" on "GENERIC_LINUX_SERVER"

  @SID_6
  Scenario: SNMP trap Generate trap
  ...
  @SID_7
  Scenario: SNMP delete alert rule
    Then CLI Operations - Run Root Session command "mysql -prad123 vision_ng -e "delete from alert_rule;""

  @SID_8
  Scenario: Verify application trap received at snmpd server
