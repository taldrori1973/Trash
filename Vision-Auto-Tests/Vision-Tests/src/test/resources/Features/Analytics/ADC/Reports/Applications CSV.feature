...
Feature: ADC Applications Generate CSV Report
  ...
  Scenario: ADC Applications report validate CSV file Groups and Content Rules widget header
    ...
    Then CLI Run linux Command "cat "/opt/radware/mgt-server/third-party/tomcat/bin/Groups\ and\ Content Rules-Application.csv"|head -8|tail -1|awk -F "," '{printf $4}';echo" on "ROOT_SERVER_CLI" and validate result EQUALS "123456789abcdefghijklmno...vwxyz"
    Then CLI Run linux Command "cat "/opt/radware/mgt-server/third-party/tomcat/bin/Groups\ and\ Content Rules-Application.csv"|head -9|tail -1|awk -F "," '{printf $4}';echo" on "ROOT_SERVER_CLI" and validate result EQUALS "abcdabcdabcdabcdabcdabcdabcdabcdabcdabcd"
