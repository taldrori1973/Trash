...
#    In HTML, the format causes "Multiple" to be cut at different times with "=". This time it was after the "M". We need to think of a way to ignore the "=" in general.
    Then CLI Run linux Command "cat /var/spool/mail/alertuser| tr -d '\n'|grep -oP "(=*)M(=*)ultiple" |wc -l" on "GENERIC_LINUX_SERVER" and validate result EQUALS "8"
