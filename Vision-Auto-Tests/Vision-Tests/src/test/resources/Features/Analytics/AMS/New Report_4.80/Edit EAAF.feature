...
Feature: Edit EAAF tests
  ...
  Scenario: Delete Template Widget from EAAF
    Given UI "Edit" Report With Name "EAAF Report"
      | Template-1 | reportType:EAAF , DeleteWidgets:[{Top Malicious IP Addresses:[Packets]}]     |
    Then UI "Validate" Report With Name "EAAF Report"
      | Template-1 | reportType:EAAF , Widgets:[{Top Attacking Geolocations:[Events]}]      |
  ...
  Scenario: Edit DefensePro Analytics Report2 report name
    ...
    Then UI Text of "Error message title" equal to "Unable To Save Report"
    Then UI Text of "Error message description" equal to "Report name must be unique. There is already another report with name 'EAAF Report2'"
    Then UI Click Button "errorMessageOK"
    Then UI Click Button "cancel"
    Then UI Text of "Save Change Message" contains "Do you want to save "EAAF Report2"?"
