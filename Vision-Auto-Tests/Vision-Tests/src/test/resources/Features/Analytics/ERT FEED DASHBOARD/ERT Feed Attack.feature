...
#    * REST Delete ES index "dp-five-*"
    ...
    * CLI Run remote linux Command "curl -X POST -H "Content-type: application/json" localhost:9200/dp-attack-raw-*/_search -d '{"query":{"bool":{"must":[{"match_all":{}}],"must_not":[],"should":[]}},"from":0,"size":1000,"sort":[],"aggs":{}}' >> /opt/radware/storage/maintenance/attack-raw-index-before-stream" on "ROOT_SERVER_CLI"
  @SID_2
  Scenario: Run DP simulator PCAPs for EAAF widgets and arrange the data for automation needs
    ...
    Then CLI copy "/home/radware/Scripts/uVision_EAAF_attacksTimeSpreadingScript.sh" from "GENERIC_LINUX_SERVER" to "ROOT_SERVER_CLI" "/"
    # copy script that keeps the attacks times in relation to system current time
    Then CLI copy "/home/radware/Scripts/uVision_EAAF_KeepAttacksTimesUpToDate.sh" from "GENERIC_LINUX_SERVER" to "ROOT_SERVER_CLI" "/"
    # run the script of attacks data time modification
    Then CLI Run remote linux Command "/uVision_EAAF_attacksTimeSpreadingScript.sh" on "ROOT_SERVER_CLI" with timeOut 1800
    ...
    Then CLI Run remote linux Command "nohup /uVision_EAAF_KeepAttacksTimesUpToDate.sh" on "ROOT_SERVER_CLI"
  ...
  Scenario: Validate Top Attacking Geolocations Widget - Packets
    ...
    Then UI Validate Text field "TOTAL Country Attacks value" with params "0" EQUALS "74 K" with offset 2
  ...
  Scenario: Validate Top Attacking Geolocations Widget - Events
    ...
    Then UI Validate Text field "TOTAL Country Attacks value" with params "0" EQUALS "1"
  ...
  Scenario: Validate Top Attacking Geolocations Widget - Volume
    ...
    Then UI Validate Text field "TOTAL Country Attacks value" with params "0" EQUALS "35 M" with offset 2
