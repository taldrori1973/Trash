...
Feature: Vision Scale APM
  ...
  Scenario: Validate simulation davices are up and running
    ...
    Then CLI Run remote linux Command "vision_ng -e "select c.mgt_ip,f.status from site_tree_elem_abs as a inner join device_setup as b on b.row_id=a.fk_dev_setup_device_setup inner join device_access as c on c.row_id=b.fk_dev_access_device_acces inner join software as d on d.row_id=b.fk_dev_sw_software inner join hardware as e on e.row_id=b.fk_hw_hardware inner join devicestatus as f on f.row_id=a.fk_dev_status where c.mgt_ip like '%50.50%' and f.status<>'1'  order by c.mgt_ip" > /opt/radware/storage/maintenance/DeviceThatWhereDown.log" on "ROOT_SERVER_CLI"
    Then CLI Run linux Command "vision_ng -NB -e "select c.mgt_ip,f.status from site_tree_elem_abs as a inner join device_setup as b on b.row_id=a.fk_dev_setup_device_setup inner join device_access as c on c.row_id=b.fk_dev_access_device_acces inner join software as d on d.row_id=b.fk_dev_sw_software inner join hardware as e on e.row_id=b.fk_hw_hardware inner join devicestatus as f on f.row_id=a.fk_dev_status where c.mgt_ip like '%50.50%' and f.status<>'1'  order by c.mgt_ip" | wc -l" on "ROOT_SERVER_CLI" and validate result EQUALS "0"
