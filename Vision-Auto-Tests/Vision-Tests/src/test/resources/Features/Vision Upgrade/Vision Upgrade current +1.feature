...
#    Then CLI Run linux Command "ip6tables -L -n |grep tcp|grep dpt:9200" on "ROOT_SERVER_CLI" and validate result CONTAINS "ACCEPT"
  ...
  Scenario: Validate TED status
    Then CLI Run linux Command "echo $(mysql -prad123 vision_ng -N -B -e "select count(*) from vision_license where license_str like '%reporting-module-ADC%';")-$(netstat -nlt |grep 5140|wc -l)|bc" on "ROOT_SERVER_CLI" and validate result EQUALS "0" with timeOut 240
    Then CLI Run linux Command "curl -ks -o null -w 'RESP_CODE:%{response_code}\n' -XGET https://localhost:443/ted/api/data" on "ROOT_SERVER_CLI" and validate result EQUALS "RESP_CODE:200" with timeOut 240
  ...
  Scenario: Validate LLS service is up
    ...
    # The LLS installation is subject to change if LLS was upgraded
      ...
      | LLS     | touching upgrade flag file /opt/radware/storage/lls/lls_is_upgrading | NOT_EXPECTED |
      | LLS     | removing upgrade flag file /opt/radware/storage/lls/lls_is_upgrading | NOT_EXPECTED |
