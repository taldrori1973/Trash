...
Feature: Vision Upgrade current -2
  ...
  Scenario: Pre Upgrade changes
    ...
    Then CLI Run remote linux Command "vision_ng -e "delete from vision_license where license_str like '%reporting-module-ADC%';"" on "ROOT_SERVER_CLI"
 #for testing AVA Attack Capacity Grace Period with the following scenario:
  ...
  Scenario: Validate TED status when no ADC license
    Then CLI Run linux Command "echo $(vision_ng -N -B -e "select count(*) from vision_license where license_str like '%reporting-module-ADC%';")-$(netstat -nlt |grep 5140|wc -l)|bc" on "ROOT_SERVER_CLI" and validate result EQUALS "0" Retry 600 seconds
