...
Feature: Elasticsearch watchdog
  ...
  Scenario: verify watchdog logs
    Then CLI Run linux Command "grep -c "elastic health-check failed" /opt/radware/storage/maintenance/logs/elasticsearch_watchdog.log" on "ROOT_SERVER_CLI" and validate result GTE "1" Retry 240 seconds
    Then CLI Run linux Command "cat /opt/radware/storage/maintenance/logs/elasticsearch_watchdog.log |grep "elastic health-check failed"| wc -l" on "ROOT_SERVER_CLI" and validate result GTE "1" Retry 240 seconds
    Then CLI Run linux Command "grep -c "killing the process and restarting elasticsearch service" /opt/radware/storage/maintenance/logs/elasticsearch_watchdog.log" on "ROOT_SERVER_CLI" and validate result EQUALS "1" Retry 240 seconds
