...
Feature:test
  ...
  Scenario: Create Report of Traffic Global Kbps Inbound
    Given UI "Create" Report With Name "Traffic Report"
      ...
      | Template-2            | reportType:DefensePro Analytics , Widgets:[{BDoS-TCP SYN:[pps,IPv6,15]},Concurrent Connections],devices:[{devicesIndex:11,devicePorts:[1,2],devicePolicies:[BDOS,APOL]},{deviceIndex:10}] |
      | Format                | Select: CSV                                                                                                                                                                            |
